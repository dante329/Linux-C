/**
 * 写饥饿解决
 * 在读写锁的初始化函数pthread_rwlock_init里面可以传入pthread_rwlockattr_t *attr参数，这个参数可以设置读写的优先级
 */
...
int main(int argc, char const *argv[])
{
    ...
    pthread_rwlockattr_init(&attr); //先初始化attr
    pthread_rwlockattr_setkind_np(&attr,PTHREAD_RWLOCK_PREFER_WRITER_NONRECURSIVE_NP); //设置写操作优先

    //动态分配读写锁
    pthread_rwlock_init(&rwlock,&attr);

    //destroy attr
    pthread_rwlock_destroy(&attr);
    ...
    pthread_create(&writer2,NULL,wirte_thread,"writer2"); //读写操作会随机进行
